...
bool IOLoginData::loadPlayer(Player* player, DBResult_ptr result) {
	...
	//LOnNE ELEMENTO 
	if (!player->setElement(result->getNumber<uint16_t>("element"))) {
		std::cout << "[Warning - IOLoginData::loadPlayer] " << player->name << " has Element ID "
...
bool IOLoginData::savePlayer(Player* player) {
	...
	query << "`element` = " << player->getElementId() << ',';
